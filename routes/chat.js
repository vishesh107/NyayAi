...
router.post('/:botKey', authMiddleware, async (req, res) => {
  ...
  if (wantsStream) {
    return handleStream(res, bot, validMessages, model, user, botKey, userMessage);
  }

  // ── 7. Standard response ──────────────────────────────────
  try {
    console.log(`[Chat] ${botKey} | ${model} | user:${user.id.slice(0,8)} | "${userMessage.slice(0,60)}..."`);
...
// ══════════════════════════════════════════════════════════════
async function handleStream(res, bot, messages, model, user, botKey, userMessage) {
